...
const { containsProfanity } = require("../lib/profanity");  //Extension 1 task
...
const create_item = (req, res) => {
  ...
  // Profantify filter on new auction items
  if (containsProfanity(req.body.name) || containsProfanity(req.body.description)) {
  return res.status(400).json({ error_message: "Item contains inappropriate language" });
  }
